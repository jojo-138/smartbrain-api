				...
				.then((loginEmail) => {
					trx('users')
						...
						.then((user) => {
							...
							localTimeJoined.setMinutes(
								localTimeJoined.getMinutes() - localTimeJoined.getTimezoneOffset()
							);
...
app.post('/signin', (req, res) => {
	...
	if (req.body.email && req.body.password) {
		knex('login')
			...
			.then((password) => {
				if (!password.length)
					return res.status(400).json('Incorrect credentials. Please try again.');
...
app.post('/apiCall', (req, res) => {
	stub.PostModelOutputs(
		...
		(err, response) => {
			...
			const regions = response.outputs[0].data.regions.map((region) => ({
				id: region.id,
